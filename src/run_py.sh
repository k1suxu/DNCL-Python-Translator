...
do
    ...
    python3 test-data/outputs/$output_file > test-data/run-outputs/${output_file%.py}.txt 2> test-data/run-error-logs/${output_file%.py}.txt
    echo "[Finish]: Successfully Run"
    echo "==================================================="
done

# combined-run-output.txt が存在する場合は削除
if [ -e test-data/combined-run-output.txt ]; then
    rm test-data/combined-run-output.txt
fi
...
# combined-run-output.txt に全ての実行ログを書き込む
for output_file in $output_files
do
    # # 空だったらスキップ
    # if [ ! -s test-data/run-outputs/${output_file%.py}.txt ]; then
    #     continue
    # fi
    echo "==================== $output_file ====================" >> test-data/combined-run-output.txt
    cat test-data/run-outputs/${output_file%.py}.txt >> test-data/combined-run-output.txt
    echo "" >> test-data/combined-run-output.txt
done
